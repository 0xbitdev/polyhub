...
// Convert various price shapes to 0-100 percent
function priceToPercent(val: unknown): number {
  const n = Number(val)
  if (!Number.isFinite(n)) return 0
  // If 0..1, treat as probability; if already 0..100, leave; otherwise clamp
  ...
  try {
    ...
    try {
      ...
      const normalized = arr.map((m) => {
        // Try to derive yes/no from outcomes/outcomePrices first
        ...
        const findIndex = (label: string) =>
          outcomes.findIndex((o) => String(o).trim().toLowerCase() === label)
        ...
        // Next, try tokens if not resolved yet
        ...
        return {
          id: String(m?.id ?? m?.slug ?? m?.marketId ?? m?.market_slug ?? crypto.randomUUID()),
